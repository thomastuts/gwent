...
$('table').each(function (index) {
  ...
  rows.each(function () {
    ...
    var typeData = data.eq(3).text();
    var cardType;
    var row;
    var isHero = typeData.indexOf('Hero') > -1;
    var isLeader = typeData === 'Leader';

    row = typeData.replace('Hero', '');
    ...
    var ability = data.eq(4).text()
      .replace(/\//g, '')
      .replace('Immune to special cards', '');

    cardData.name = data.eq(0).text();
    cardData.slug = slugify(cardData.name);
    cardData.picture = data.eq(1).find('img').attr('src');
    cardData.type = cardType;

    if (cardData.type === 'Hero' || cardData.type === 'Unit') {
      cardData.row = row;
    }
